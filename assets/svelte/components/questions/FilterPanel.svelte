<script>
    let { difficultyRange = $bindable([1, 5]), answerStatus = $bindable('all') } = $props();
...
<div class="bg-base-200/50 rounded-lg p-4 space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <!-- Difficulty Range -->
        <div class="space-y-3">
            <label class="text-sm font-medium text-base-content">Difficulty Level</label>
            <div class="space-y-3">
                <!-- Range Display -->
                ...
                <!-- Min Range -->
                <div class="space-y-1">
                    <label class="text-xs text-base-content/60">Minimum</label>
                    <input 
                        type="range" 
                        min="1" 
                        max="5" 
                    ...
                    />
                    <div class="w-full flex justify-between text-xs px-2 text-base-content/40">
                        <span>1</span>
                        <span>2</span>
                        <span>3</span>
                        <span>4</span>
                ...
                <!-- Max Range -->
                <div class="space-y-1">
                    <label class="text-xs text-base-content/60">Maximum</label>
                    <input 
                        type="range" 
                        min="1" 
                        max="5" 
        ...
        <!-- Answer Status -->
        <div class="space-y-3">
            <label class="text-sm font-medium text-base-content">Answer Status</label>
            <div class="space-y-2">
    ...
    <div class="flex flex-wrap gap-2 pt-2 border-t border-base-300">
        ...
        {#if answerStatus !== 'all'}
            <div class="badge badge-secondary gap-1">
                <span>{answerStatus === 'answered' ? 'Answered Only' : 'Unanswered Only'}</span>
            </div>
        {/if}
