...
<div class="space-y-4">
    ...
    <div class="space-y-3">
        {#each data.options as option, index}
            ...
            {@const isCorrectOption = isAnswered && data.correct_indices.includes(index)}
            {@const isIncorrectSelection = isAnswered && isSelected && !data.correct_indices.includes(index)}
            {@const isMissedCorrect = isAnswered && !isSelected && data.correct_indices.includes(index)}
                        ...
                        {#if isAnswered}
                            <div class="flex items-center gap-1">
                                {#if isCorrectOption && isSelected}
                                    ...
                                    <span class="text-xs text-success font-medium">Correct</span>
                                {:else if isIncorrectSelection}
                                    <svg class="w-4 h-4 text-error" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                                    </svg>
                                    <span class="text-xs text-error font-medium">Incorrect</span>
                                {:else if isMissedCorrect}
                                    ...
                                    <span class="text-xs text-warning font-medium">Missed</span>
                                {/if}
                            </div>
                        {/if}
                    </div>
    ...
    {#if isAnswered}
        ...
        {@const correctCount = selectedOptions.filter(i => data.correct_indices.includes(i)).length}
        {@const incorrectCount = selectedOptions.filter(i => !data.correct_indices.includes(i)).length}
        {@const missedCount = data.correct_indices.filter(i => !selectedOptions.includes(i)).length}
        ...
        <div class="p-4 rounded-lg {isCorrect ? 'bg-success/10 border border-success/20' : 'bg-base-100 border border-base-300'}">
            <div class="flex items-start gap-3">
                {#if isCorrect}
                    ...
                    <div>
                        <div class="font-medium text-success">Perfect! All correct</div>
                        <div class="text-sm text-base-content/70 mt-1">
                            You selected {selectedOptions.length} of {data.correct_indices.length} correct answers
                        </div>
                    </div>
                {:else}
                    ...
                    <div class="flex-1">
                        <div class="font-medium text-error mb-2">Review your answers</div>
