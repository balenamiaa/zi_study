<script>
    ...
    function handleMatchChange(premiseIndex, optionIndex) {
        if (!isAnswered && !isSubmitting) {
            ...
            const existingMatchIndex = newMatches.findIndex(
                (match) => match.premise_index === premiseIndex,
            );
    ...
    function getSelectedOptionForPremise(premiseIndex) {
        const match = selectedMatches.find(
            (m) => m.premise_index === premiseIndex,
        );
...
<div class="space-y-4">
    ...
    {#if data.instructions}
        <div
            class="text-sm text-base-content/70 italic bg-base-200/50 p-3 rounded-lg"
        >
            {data.instructions}
        </div>
    {/if}
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="space-y-4">
            <div
                class="text-base font-semibold text-base-content border-b border-base-300 pb-2"
            >
                Premises
            </div>
            <div class="space-y-3">
                {#each data.premises as premise, premiseIndex}
                    ...
                    <div
                        class="p-4 rounded-lg border transition-all duration-200 {isCorrect
                    ...
                    >
                        <div class="flex items-start gap-3">
                            ...
                            <div class="flex-1">
                                <div
                                    class="text-sm text-base-content leading-relaxed"
                                >
                                    {premise}
                                </div>
                ...
                <div class="grid grid-cols-1 gap-2">
                    {#each data.options as option, optionIndex}
                        <div class="flex items-center gap-2 text-sm">
                            <div
                                class="w-6 h-6 bg-secondary text-secondary-content rounded-full flex items-center justify-center text-xs font-bold"
                            >
            ...
            <div class="space-y-3">
                {#each data.premises as _, premiseIndex}
                    {@const selectedOption =
                        getSelectedOptionForPremise(premiseIndex)}
                    {@const isCorrect = isMatchCorrect(premiseIndex)}
                    {@const isIncorrect = isMatchIncorrect(premiseIndex)}

                    <div
                        class="flex items-center gap-3 p-3 rounded-lg border {isCorrect
                            ? 'border-success bg-success/5'
                            : isIncorrect
                              ? 'border-error bg-error/5'
                              : 'border-base-300'}"
                    >
                        ...
                        <select
                            ...
                            onchange={(e) =>
                                handleMatchChange(premiseIndex, e.target.value)}
                            disabled={isAnswered || isSubmitting}
                            class="select select-bordered flex-1 {isCorrect
                                ? 'select-success'
                                : isIncorrect
                                  ? 'select-error'
                                  : 'select-primary'}"
            ...
            <div class="flex items-start gap-3">
                ...
                {:else}
                    ...
                    <div>
                        ...
                        <div class="text-sm text-base-content/70 mt-1">
                            {correctCount} of {data.premises.length} matches correct.
                            Check the corrections above.
                        </div>
                    </div>
                {/if}
            </div>
